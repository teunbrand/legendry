...
The goal of legendry's `guide_upset()` is to display the combination matrix at the axis, without pidgeonholing what the rest of the plot can be.
...
The 'magic' of `guide_upset()` is really the `key_upset()` that figures out the set membership.
...
The `guide_upset()` also has a sibling-guide called `guide_symbols()`.
In principle, it can do all the things that `guide_upset()` can do, but it is geared towards manually setting up a matrix of symbols.
