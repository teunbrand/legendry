...
```{r}
staxis <- compose_stack("axis_custom", "axis_custom", "axis_custom")
...
At this point, it might be wise to take a closer look at exactly what `guide_axis_custom()` is doing.
Once we pull on the threads on what its class is, the mystery starts to unravel:

```{r}
class(guide_axis_custom())
```

Indeed, by using `compose_stack("axis_custom", "axis_custom", ...)` we have been stacking a stack!
So when we peel back the layers and see what is inside `guide_axis_custom()`, will we find more stacks?
...
For example, the `guide_axis_custom()` function is a stack of three primitives.
...
Similar to how we naively believed `guide_axis_stack()` to its own little snowflake of a guide, so too is `guide_colbar()` in fact a composition 
...
The mechanism is in place in guides like `guide_axis()`/`guide_axis_custom()` for display in for example `facet_grid(axis.labels = "margins")`.
...
A gizmo differs from a primitive in that a gizmo is not a general building block, but rather a very specialised display of a particular aesthetic.
...
standard + 
  ...
  guides(
    ...
    colour = compose_sandwich(middle = my_gizmo, text = "axis_custom")
  )
```
...
standard +
  ...
  scale_colour_viridis_c(
    guide = compose_crux(
      centre = gizmo_barcap(), 
      left = "axis_custom", right = "axis_custom",
